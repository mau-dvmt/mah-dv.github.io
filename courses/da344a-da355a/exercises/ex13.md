...
Ni skickar enklast ett anrop genom att skicka iväg ett formulär med metoden ajaxSubmit (plugin, [ni hittar det här](http://jquery.malsup.com/form/), källkoden (som ni behöver inkludera) [finns här](http://malsup.github.io/jquery.form.js)). Det som krävs från server är (skicks med metoden `POST`):
